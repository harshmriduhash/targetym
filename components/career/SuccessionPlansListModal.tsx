...
export default function SuccessionPlansListModal({ open, onOpenChange, plans }: SuccessionPlansListModalProps) {
  ...
  const getRiskBadge = (riskLevel: string) => {
    switch (riskLevel) {
      case 'low':
        return <Badge className="bg-green-100 text-green-700 hover:bg-green-100">Faible</Badge>;
      case 'medium':
        return <Badge className="bg-yellow-100 text-yellow-700 hover:bg-yellow-100">Moyen</Badge>;
      case 'high':
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh]">
        ...
        <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-2">
          {plans.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">Aucun plan de succession créé</p>
          ) : (
            plans.map((plan) => (
              <div key={plan.id} className="p-4 border rounded-lg hover:bg-accent/50 transition-colors">
                ...
                <div className="bg-blue-50 dark:bg-blue-950/20 p-3 rounded-lg mb-3">
                  ...
                  <p className="text-sm">
                    ...
                    <span className="text-muted-foreground"> - {plan.successorCurrentRole}</span>
                  </p>
                  {plan.timeline && (
                ...
                {plan.gapsToAddress && (
                  <div className="mb-3">
                    <p className="text-xs font-medium text-muted-foreground mb-1">Écarts à combler</p>
                ...
                {plan.developmentPlan && (
                  <div className="mb-3">
                    <p className="text-xs font-medium text-muted-foreground mb-1">Plan de développement</p>
                    <p className="text-sm line-clamp-2">{plan.developmentPlan}</p>
                  </div>
                )}

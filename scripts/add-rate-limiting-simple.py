...
APPROCHE SIMPLE ET SURE - utilise le pattern decorator sans modifier le body
"""
...
RATE_LIMIT_IMPORT = "import { withRateLimit } from '@/src/lib/middleware/action-rate-limit'"
...
def convert_to_decorator_pattern(content: str, rate_limit_type: str) -> Tuple[str, str]:
    """
    Convertit:
    export async function myFunc(input: T): Promise<R> { ... }

    En:
    export const myFunc = withRateLimit('type', async (input: T): Promise<R> => { ... })
    """

    # Pattern pour capturer la declaration complete de la fonction
    # Capture: nom, parametres, type de retour
    ...
    # Trouver l'accolade fermante de la fonction (en comptant les accolades)
    ...
    end_pos = pos  # Position apres le }

    # Extraire tout le corps de la fonction (avec les accolades)
    function_body = content[start_pos:end_pos]

    # Creer la nouvelle declaration avec decorator
    new_declaration = f"export const {function_name} = withRateLimit('{rate_limit_type}', async {params}{return_type} =>"

    # Remplacer l'ancienne declaration
    new_content = content[:match.start()] + new_declaration + function_body + ')' + content[end_pos:]
...
def process_file(filepath: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Determiner le type de rate limit (avec un nom temporaire pour l'analyse)
        ...
        # Convertir au pattern decorator
        content, converted_name = convert_to_decorator_pattern(content, rate_limit_type)

        if converted_name is None:
            print(f"  [ERR] Echec de la conversion")
...
def main():
    ...
    print("=" * 60)
    print("AJOUT AUTOMATIQUE DE RATE LIMITING (PATTERN DECORATOR)")
    print("=" * 60)

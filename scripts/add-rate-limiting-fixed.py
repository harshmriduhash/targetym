...
VERSION CORRIGEE - gere correctement les types complexes
"""
...
def extract_function_info(content: str) -> Tuple[str, int, int]:
    ...
    CORRIGE: Trouve correctement l'accolade du BODY, pas celle du type de retour
    """

    # Pattern pour trouver export async function avec son type de retour
    # Capture jusqu'au dernier ) avant l'accolade du body
    ...
    # Trouver l'accolade ouvrante du BODY (apres le type de retour)
    # On doit sauter les { } dans le type de retour comme Promise<ActionResponse<{ id: string }>>
    type_part = match.group(2)

    # Position apres la signature complete
    signature_end = match.end()

    # Chercher l'accolade du body apres la signature
    brace_start = content.index('{', signature_end)
...
def main():
    ...
    print("=" * 60)
    print("AJOUT AUTOMATIQUE DE RATE LIMITING (VERSION CORRIGEE)")
    print("=" * 60)
